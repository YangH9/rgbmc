...
function updateOutputText(event) {
  ...
  if (document.getElementById('output-format').value == 9) {
    output = format.outputPrefix;

    $('#hexColors').find('.hexColor').each((index, element) => {
      output = output + ":" + $(element).val();
    })
    let formatCodes = '';
    if (format.formatChar != null) {
      if (bold) formatCodes += format.formatChar + 'l';
